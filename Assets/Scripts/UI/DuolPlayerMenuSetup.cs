...
namespace DuolBots
{
    public class DuolPlayerMenuSetup : MonoBehaviour
    {
        ...
        public GameObject GetCorrespondingElement(int playerIndex)
        {
            if (playerIndex == 0)
                return leftBeginElement;
            if (playerIndex == 1)
                return rightBeginElement;

            Debug.Log("Invalid Player Index " + playerIndex);
            return null;
        }

        public void SetupMenuForPlayer(int playerIndex)
        {
            GameObject[] temp_playerInputs =
                   GameObject.FindGameObjectsWithTag(m_playerInputTag);
            Assert.AreEqual(2, temp_playerInputs.Length, $"Expected to find " +
                $"2 players, instead found {temp_playerInputs.Length}");
